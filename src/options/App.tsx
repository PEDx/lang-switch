...
function ProviderEditor({ value, onChange, onSave, onDelete }: { value: ProviderConfig; onChange: (value: ProviderConfig) => void; onSave: () => Promise<void>; onDelete: () => void }) {
  ...
  return (
    <section className="card provider-editor">
      ...
      <fieldset><legend>API 类型</legend><label className="radio"><input type="radio" checked={value.type === 'anthropic'} onChange={() => changeType('anthropic')} /> Anthropic Messages</label><label className="radio"><input type="radio" checked={value.type === 'openai-responses'} onChange={() => changeType('openai-responses')} /> OpenAI Responses</label><label className="radio"><input type="radio" checked={value.type === 'openai-compatible'} onChange={() => changeType('openai-compatible')} /> OpenAI Compatible</label></fieldset>
...
export function App() {
  ...
  const removeProvider = () => {
    if (!selected) return
    const next = providers.filter((item) => item.id !== selected.id)
    const nextStageIds = Object.fromEntries(
      Object.entries(settings.translationStageProviderIds ?? {})
        .filter(([, providerId]) => providerId !== selected.id),
    )
    setProviders(next)
    setSelectedId(next[0]?.id ?? '')
    setSettings({
      ...settings,
      primaryProviderId: settings.primaryProviderId === selected.id ? next[0]?.id : settings.primaryProviderId,
      translationStageProviderIds: nextStageIds,
    })
  }
  const setStageProvider = (stage: 'analysis' | 'initial' | 'review' | 'refinement', providerId: string) => {
    const next = { ...settings.translationStageProviderIds }
    if (providerId) next[stage] = providerId
    else delete next[stage]
    setSettings({ ...settings, translationStageProviderIds: next })
  }
  const providerOptions = providers.map((provider) => (
    <option key={provider.id} value={provider.id}>{provider.name} · {provider.model || '未设置模型'}</option>
  ))
  return (
    <main className="options-shell">
      ...
      <div className="layout"><nav className="sidebar"><button className="add" onClick={addProvider}>＋ 新建 Provider</button>{providers.map((provider) => <button key={provider.id} className={provider.id === selectedId ? 'active' : ''} onClick={() => setSelectedId(provider.id)}><span>{provider.type === 'anthropic' ? 'A' : provider.type === 'openai-responses' ? 'R' : 'O'}</span><div>{provider.name}<small>{provider.model || '尚未设置模型'}</small></div></button>)}</nav><div>
        {selected ? <ProviderEditor key={selected.id} value={selected} onChange={updateSelected} onSave={persist} onDelete={removeProvider} /> : <section className="card empty"><h2>还没有 Provider</h2><p>创建一个 OpenAI Responses、OpenAI Compatible 或 Anthropic 配置后即可开始精译。</p><button className="primary" onClick={addProvider}>新建 Provider</button></section>}
        <section className="card"><span className="eyebrow">基础设置</span><h2>阅读偏好</h2><div className="form-grid"><label>主 Provider<select value={settings.primaryProviderId ?? ''} onChange={(event) => setSettings({ ...settings, primaryProviderId: event.target.value || undefined })}><option value="">自动选择第一个</option>{providerOptions}</select></label><label>默认目标语言<select value={settings.defaultTargetLanguage} onChange={(event) => setSettings({ ...settings, defaultTargetLanguage: event.target.value })}><option value="zh-CN">简体中文</option><option value="zh-TW">繁體中文</option><option value="ja">日本語</option><option value="en">English</option></select></label><label>原文透明度<input type="range" min="10" max="100" value={settings.originalOpacity * 100} onChange={(event) => setSettings({ ...settings, originalOpacity: Number(event.target.value) / 100 })} /></label><label className="check"><input type="checkbox" checked={settings.autoUseSiteRules} onChange={(event) => setSettings({ ...settings, autoUseSiteRules: event.target.checked })} /> 自动使用站点规则</label><label className="check"><input type="checkbox" checked={settings.showSegmentToolbar} onChange={(event) => setSettings({ ...settings, showSegmentToolbar: event.target.checked })} /> 显示单段工具栏</label></div></section>
        <section className="card"><span className="eyebrow">模型路由</span><h2>翻译阶段模型</h2><p className="section-description">默认全部继承主 Provider。只为需要更高质量或更低成本的阶段设置覆盖。</p><div className="form-grid"><label>文章分析<select value={settings.translationStageProviderIds?.analysis ?? ''} onChange={(event) => setStageProvider('analysis', event.target.value)}><option value="">继承主 Provider</option>{providerOptions}</select></label><label>初始翻译<select value={settings.translationStageProviderIds?.initial ?? ''} onChange={(event) => setStageProvider('initial', event.target.value)}><option value="">继承主 Provider</option>{providerOptions}</select></label><label>翻译审阅<select value={settings.translationStageProviderIds?.review ?? ''} onChange={(event) => setStageProvider('review', event.target.value)}><option value="">继承主 Provider</option>{providerOptions}</select></label><label>最终润色<select value={settings.translationStageProviderIds?.refinement ?? ''} onChange={(event) => setStageProvider('refinement', event.target.value)}><option value="">继承主 Provider</option>{providerOptions}</select></label></div></section>
